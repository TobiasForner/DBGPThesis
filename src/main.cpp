...
int main(int argc, char **argv) {
    ...
    for (int runNr = 0; runNr < nrRuns; runNr++) {
        ...
        if (method == "decompTree") {
            ...
            try {
                ...
                if (delMethodString == "OLD") {
                    ...
                    std::cout << "deletionMethod,CORE_HALO_FILTER" << std::endl;
                    //spdlog::info("Deletion method set to CORE_HALO_FILTER.");
                } else {
                    std::cout << "Invalid state in deletion method specification reached. Shutting down." << std::endl;
            ...
            decompositionMethod = std::make_unique<decomposition_tree::DecompositionTree>(
                    ...
                    input.getDoubleVal("augmentation"), input.getIntVal("clusterSize"),
        ...
        if (input.getVal("dataSource") == "MongoDBData") {
            ...
            data = std::make_unique<MongoDBDataExperimental>(input.getVal("uri"), input.getVal("database"),
                                                             input.getVal("collection"), input.getIntVal("batch-size"),
        ...
        if (dynamic) {
            performDynamicRun(data.get(), decompositionMethod.get(), migrationNr, communicationNr, output_frequency,
                              ...
                              input.getDoubleVal("augmentation"));
...
#ifdef USE_DEBUG
        std::cout << "invalid matchings in run " + std::to_string(runNr) + ": " + std::to_string(invalid_matching_count)
                  << std::endl;
    ...
    std::cout << "Performed " + std::to_string(nrRuns) + " runs. Shutting down..." << std::endl;

    //spdlog::info("Performed " + std::to_string(nrRuns) + " runs. Shutting down...");
    return 0;
}