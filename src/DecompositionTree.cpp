...
namespace decomposition_tree {
    ...
    DecompositionTree::DecompositionTree(unsigned int n, unsigned const int alpha, double augmentation,
            ...
            insertBetweenServers(insertBetweenServers), deletionMethod(del_method), compGraphConstruction(comp_const),
            root(nullptr),
            leaves(std::vector<TreeLeaf>()),
            nodesToLeaves(std::vector<node_>()),
            edges(std::vector<std::shared_ptr<DoublyLinkedList<Edge *>>>()),
    ...
    void DecompositionTree::insertAndUpdate(node_ a, node_ b) {
        ...
        clusterMapping.updateMapping(alphaConnectedComponents, toDel);
