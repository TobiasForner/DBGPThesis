...
\begin{document}
	...
	\begin{abstract}
		...
		We provide a polynomial-time deterministic algorithm for this problem which achieves a competitive ratio of $O(2/\epsilon\cdot k\log k)$ and evaluate its results and performance by comparing it with the results of another algorithm variant and with existing implementations.
...
%\setlength{\floatsep}{2pt}
	...
	\begin{itemize}
		\item \textbf{Buy or Rent}(\cite{Epstein2014}): should a request be served remotely at a small cost (rent) or should the communicating nodes be collocated at a large cost up-front(buy)?
		\item \textbf{How should nodes be collocated?} If there are nodes which communicate frequently it seems advantageous to collocate them. Then the question is how one could define such a threshold and also which node should be moved in order to migrate as little as possible.
...
\label{run_time_sec}
In this section we discuss polynomial the running time of an update of the decomposition tree after a new request has been received in our implementation of \adjDel{}.
...
\textit{Proof.} The worst case is in this case given when the whole tree has to be recomputed. We first discuss the time complexity of decomposing a single tree node $v$. Let the corresponding subgraph be denoted by $G_v=(V_v, E_v)$.
...
We run \adapt{} in an online way while maintaining a communication graph by inserting an edge or incrementing a corresponding edge weight for each request.
