...
In the following lemma we combine the results of the previous lemmas in order to bound the cost of \opt{}.
...
\textit{Proof.} We combine the results from \cref{offl_int_lemma} and \cref{core_r_lemma}. Note that the cost from \cref{core_r_lemma} may contain migration costs. In this case the corresponding migrations represent the end of an offline interval. This gives us that

\begin{align*}
2\opt{}(\sigma)\geq\sum_{C\in\del{}(\sigma)}|C|/k\cdot\alpha+|\halo(C)|/k.
\end{align*}
as we account for each migration at most twice. Hence the lemma follows.\qed
