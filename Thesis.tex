...
%\setlength{\floatsep}{2pt}
	...
	We then formally introduce the Dynamic Balanced Graph Partitioning (DBGP) problem in \cref{problem_definition_section}. 
	In \cref{related_work_section} we discuss previous work related to the DBGP problem. After that we discuss two algorithmic ideas, denoted by \adjDel{} and \coreDel{}, which are inspired by prior work (\cite{Avin2015, Avin2016}) and the respective implications and challenges for their competitive analysis. 
	...
	Then we empirically evaluate our algorithms by comparing their results with those of existing implementations in \cref{evaluation_section}.
	...
	There are several algorithms and frameworks for graph partitioning problems.
	
	
	\section{Algorithmic Ideas}
	\label{algIdeas}
	In this section we describe two different solution approaches to the Dynamic Balanced Graph Partitioning problem. We call these approaches \coreDel{} and \nl\adjDel{} that are inspired by the versions of the \crep{} algorithm in \cite{Avin2015} and \cite{Avin2016}. We first describe the general approach that is common to both algorithms and then address the specific differences and analysis ideas.
...
\end{algorithm}
	...
	Our connectivity-based approach however allows us to achieve a a polynomial runtime.
...
Furthermore we know that a component deleted by \adjDel{} is both at least $\alpha$-connected as well as larger than $k$, i.e. its nodes must be spread across several servers in the solution of \opt{}. This allows assign significant cost to \opt{}.
...
In both cases only edges representing either requests from $\optreq()$ or migrations from $\optmig(C)$ were cut.

As the sets $core(C)$, $R(C)$ , $core(D)$ and $R(D)$ are disjoint per their definition we conclude that
\begin{align*}
\sum_{C\in\del(\sigma)}\optmig(C)+\optreq(C)\geq |C|/k\cdot\alpha+|R(C)|/k.
\end{align*}
\qed\nl
...
We continue the analysis of the lower bound on $\opt{}$ by bounding the cost of \opt{} due to requests we have not accounted for, namely those from $\core(C)$ and those from $R(C)$ for each deleted component $C$.

\begin{lemma}
	\label{core_r_lemma}
	Consider only the requests of sequence $\sigma$ which are contained in $core(C)\cup R(C)$ for a deleted component $C\in\del(\sigma)$. Then \opt{} pays at least
	\begin{align*}
		\sum_{C\in\del(\sigma)}|C|/k\cdot\alpha+R(C)/k
	\end{align*}
	in the course of serving these requests.%this subsequence $\sigma'$ of $\sigma$.	
\end{lemma}
...
As the sets $core(C)$, $R(C)$ , $core(D)$ and $R(D)$ are disjoint per their definition we conclude that \opt{} has to pay at least cost 
\begin{align*}
\sum_{C\in\del(\sigma)}|C|/k\cdot\alpha+R(C)/k.
\end{align*}
\qed\nl
...
\textit{Proof.} The theorem follows from the \cref{dec_run_time_lem} and the fact that the remaining subroutines can hence also be implemented in polynomial time. Note in particular that there can be at most one component deletion during the update step after any given request.\qed%TODO list other subroutines
