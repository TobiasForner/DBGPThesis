...
\pagenumbering{arabic} 
	...
	%We have also included a pseudocode description of this general outline of the algorithms in \cref{highLevelAlg}. Please note that the subroutine \textit{delete(Y)} of a component set $Y$ is different for each of the algorithms.
...
\end{lemma}
%TODO make this proof more understandable by starting with a more general statement and plugging in the previous lemmas
\textit{Proof.} We sum the results from \cref{crep_req_bound} and \cref{crep_mig_bound}:
\begin{align*}
...
&\leq 2\cdot\sum_{C\in\del{}(\sigma)\cup\finalComps(\sigma)}|C|\cdot((2/\epsilon+1)+\log k)\cdot\alpha+\sum_{C\in\del{}(\sigma)}|\halo(C)|+\finalWeights(\sigma).
\end{align*}
\qed
