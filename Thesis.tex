...
\begin{lemma}
	...
	With augmentation $2+\epsilon$, \crep{} pays at most cost $2\cdot\sum_{C\in\del{}(\sigma)}|C|\cdot((2/\epsilon+1)+\log k)\cdot\alpha+|\halo(C)|+2\cdot\sum_{C\in \finalComps(\sigma)}|C|\cdot((2/\epsilon+1)+\log k)\cdot\alpha+\finalWeights$.
\end{lemma}
